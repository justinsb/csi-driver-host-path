...
const (
	TagVolumeID   = "lvm.csi.justinsb.com/id"
	TagVolumeType = "lvm.csi.justinsb.com/type"
)

func (s *hostPath) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (resp *csi.CreateVolumeResponse, finalErr error) {
	...
	case state.MountAccess:
		// ok
	default:
		return nil, status.Errorf(codes.Unimplemented, "block devices not implemented")
	}

	lvName := req.GetName() // TODO: Sanitize?
	...
	if s.config.EnableTopology {
		topologies = append(topologies, &csi.Topology{Segments: map[string]string{TopologyKeyNode: s.config.NodeID}})
	}
	...
	if existing, err := s.lvm.findVolumeByLVName(ctx, lvName); err == nil && existing != nil {
		// Since err is nil, it means the volume with the same name already exists
		// need to check if the size of existing volume is the same as in new
		// request
		volumeSize, err := existing.VolumeSizeBytes()
		if err != nil {
			return nil, status.Errorf(codes.Internal, "error getting size of volume: %v", err)
		}
		if volumeSize != capacity {
			return nil, status.Errorf(codes.AlreadyExists, "volume with the same name: %s but with different size already exist", req.GetName())
		}
		if req.GetVolumeContentSource() != nil {
			return nil, status.Errorf(codes.Unimplemented, "volumeContentSource not implemented")
	...
	// volumeID, req.GetName(), capacity, requestedAccessType, false /* ephemeral */, kind

	size := strconv.FormatInt(capacity, 10) + "b"

	tags := []string{TagVolumeType + "=" + "fs"}
	created, err := s.lvm.createThinLV(ctx, lvName, size, tags)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "error creating volume: %v", err)
	}

	klog.V(4).Infof("created LV %s", created.LogicalVolumeName)

	if req.GetVolumeContentSource() != nil {
		return nil, status.Error(codes.Unimplemented, "VolumeContentSource not implemented")
...
func (s *hostPath) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	// Check arguments
	...
	existing, err := s.lvm.findVolumeByVolumeID(ctx, volID)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "error fetching volume %q: %v", volID, err)
	}
	if existing == nil {
		// Already deleted?
		klog.Warning("volume %q was not found in DeleteVolume; assuming already deleted")
